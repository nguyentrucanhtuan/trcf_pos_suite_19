...
class TrcfMinvoicePosOrder(models.Model):
        ...
        if not minvoice_tax_code or not minvoice_invoice_series or not minvoice_api_token:
            return {
                ...
                'params': {
                    ...
                    'message': _('Cần điền thông tin VAT INVOICE mới được phát hành hoá đơn'),
                    'type': 'warning',
                    'sticky': False,
                }
            }
    ...
    def _send_single_vat_invoice(self):
        ...
        try:
            ...
            _logger.info(f"🔑 Headers: {{'Content-Type': 'application/json', 'Authorization': 'Bearer {minvoice_api_token[:10]}...'}}")
            ...
            if result.get('ok') and result.get('code') == '00':
                ...
                _logger.info(f"✅ Order {order.pos_reference}: Phát hành thành công - Mã: {sobaomat}")
                return {
                    'success': True,
                    'vat_code': sobaomat
                }
                ...
                _logger.warning(f"⚠️ Order {order.pos_reference}: API trả về lỗi - {error_msg}")
                return {
                    'success': False,
                    'error': error_msg
                }
